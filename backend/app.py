...
def add_item():
    try:
        ...
        date_type = data.get('date_type', 'bought') # 'bought' or 'expiry'
        input_date = data.get('input_date')

        if not name or not quantity or not input_date:
            return jsonify({"error": "Missing fields"}), 400
        ...
        # ✅ SMART EXPIRY LOGIC
        if date_type == 'expiry':
            # User provided the exact expiry date
            expiry = parsed_date
            added = datetime.today() # Just mark it as added today
        else:
            # User provided the bought date, calculate expiry automatically
            added = parsed_date
...
def get_recommendations():
    ...
    cuisine_preference = data.get('cuisine', '') # ADDED THIS
    ...
    # ADDED THESE FILTERS
    if diet_preference: params["diet"] = diet_preference
    if cuisine_preference: params["cuisine"] = cuisine_preference
    ...
    for recipe in raw_recipes:
        ...
        missed_items = recipe.get("missedIngredients", [])
        missing_names_str = ", ".join([item.get("name", "") for item in missed_items])
        ...
        processed_recipes.append({
            ...
            "matchTier": tier,
            "missingCount": missed,
            "missingIngredientsList": missing_names_str, 
...
@app.route('/api/leftovers/generate', methods=['POST'])
def generate_leftover_recipes():
    data = request.get_json()
    leftovers_text = data.get('leftovers', "")
    diet_pref = data.get('diet', "")

    if not leftovers_text:
        return jsonify({"error": "Please provide some leftovers."}), 400

    # 1. Grab everything in the user's pantry
    pantry_items = PantryItem.query.all()
    pantry_str = ", ".join([item.name for item in pantry_items])
    if not pantry_str:
        pantry_str = "No extra raw ingredients available."
    ...
    # 3. The Prompt Engineering for the AI Chef
    prompt = f"""
    You are an expert zero-waste chef. I have these cooked leftovers: {leftovers_text}.
    I also have these raw ingredients in my pantry: {pantry_str}.
    My dietary preference is: {diet_pref if diet_pref else 'Any'}.
    
    Invent 3 creative, delicious meals I can make right now combining my leftovers with my pantry items.
    ...
    {{
        ...
        "readyInMinutes": 15,
        "matchPercentage": 99,
        "matchTier": "Zero Waste Match",
        "missingCount": 0,
        "instructions": "A brief 2-sentence summary of how to throw it together.",
        "sourceUrl": "#"
    }}
    """
    ...
    try:
        ...
        # Clean the response to ensure it parses as JSON perfectly
        res_text = response.text.replace('```json', '').replace('```', '').strip()
        recipes = json.loads(res_text)
        
        # Inject a placeholder food image so the React frontend cards still look beautiful
        for r in recipes:
            # Using a generic delicious food placeholder from Unsplash
            r["image"] = "[https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=500&q=80](https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=500&q=80)"
            if "id" not in r:
                r["id"] = random.randint(1000, 9999)
                
        return jsonify({"status": "success", "recipes": recipes})
        
    except Exception as e:
        print(f"Gemini API Error: {e}")
        return jsonify({"error": "AI Chef failed to generate recipes. Try again!"}), 500
